# -*- coding: utf-8 -*-
"""Iterative PLS (FTIR-NIR)_Sem preprocessamento_com inferencia_v3.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12E-mjwUYnQehiaoffFbNYCR8n6OqF3sH
"""
...
features = data.iloc[:, 4:]  # Assuming wavelength data starts at column E
   ...
   # Function to manually select wavelengths, perform PCA, and fit PLS regression
def manual_wavelength_selection_pca_pls(features, target, start_col, end_col, n_components=3):
...
start_col = 10
end_col = 60
pca_df, explained_variance, pls_model, scaler, r2, rmse = manual_wavelength_selection_pca_pls(features, target, start_col, end_col)

# Print the PLS model performance
...
n_components = 3  # Adjust the number of latent variables as needed
scaler = StandardScaler()
...
print(f"PLS Regression on Full Spectrum: R2 = {r2:.4f}, RMSE = {rmse:.4f}, Latent Variables = {n_components}")
...
plt.xlabel('Actual Concentrations')
plt.ylabel('Predicted Concentrations')
plt.title('PLS Regression: Actual vs Predicted Concentrations (Full Spectrum)')
plt.legend()
plt.grid(True)
plt.show()

# Function to evaluate a band with PLS
def evaluate_band_pls(start_col, band_width, n_components=3):
    ...
    y_pred = pls.predict(X_test_scaled).flatten()
    mse = mean_squared_error(y_test, y_pred)
    rmse = np.sqrt(mse)  # Calculate RMSE manually

    return r2_score(y_test, y_pred), y_test, y_pred, X_train_scaled, scaler, pls, rmse, X_train, y_train # Return y_test and y_pred
...
for band_width in band_widths:
    for start_col in range(0, features.shape[1] - band_width + 1, band_width):  # Slide over features
        r2, y_test, y_pred, X_train_scaled, scaler, pls, rmse, X_train, y_train = evaluate_band_pls(start_col, band_width, n_components=n_components) # Receive rmse from the function call
        ...
        if r2 > best_r2:
            ...
            # Store the best PLS model
            best_pls_model = pls  # Assign the current 'pls' to 'best_pls_model'
...
inference_data = pd.read_csv('_Inferencias compostas (ureia).csv', delimiter=';', encoding='latin-1')
...
training_columns = features.columns[best_band[0]:best_band[0] + best_band[1]]
...
# Perform prediction
predicted_conc = loaded_model[0].predict(inference_scaled).flatten()
