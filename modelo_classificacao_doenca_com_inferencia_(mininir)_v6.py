# -*- coding: utf-8 -*-
"""Modelo classificacao Doenca com inferencia (miniNIR)_v6.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1N4MwQ_C0DNHF9cbnJ5EI2ugX_bz8otdH
"""
...
# Load the dataset
df = pd.read_csv('2025.02.13-miniNir (solucoes simples)-Creatinina-Doenca.csv', delimiter=';', encoding='latin-1', decimal=',')

# Extract X (columns D to I) and y (column C)
X = df.iloc[:, 4:10].replace(',', '.', regex=True).astype(float)  # Convert to numeric
y = df['Doenca']  # Column C as target variable

# Encode target labels to classes
le = LabelEncoder()
y_encoded = le.fit_transform(df["Doenca"])
...
def evaluate_model_with_specificity(model, model_name, X_train, X_test, y_train, y_test):
    ...
    if y_train_proba is not None:
        if len(np.unique(y_train)) > 2:  # Check for multiclass
           auc_train = roc_auc_score(y_train, y_train_proba[:, 1], multi_class='ovr')  # Use probability of positive class for ovr calculation
        else:
            auc_train = roc_auc_score(y_train, y_train_proba[:, 1]) # Use probability of positive class for binary calculation
    ...
    if y_test_proba is not None:
        if len(np.unique(y_test)) > 2:  # Check for multiclass
            auc_test = roc_auc_score(y_test, y_test_proba[:, 1], multi_class='ovr')  # Use probability of positive class for ovr calculation
        else:
           auc_test = roc_auc_score(y_test, y_test_proba[:, 1]) # Use probability of positive class for binary calculation
    else:
         auc_test = None
...
def calculate_auc_ci(model, X_train, y_train, X_val, y_val, n_bootstraps=1000, confidence_level=0.95):
    ...
    for _ in range(n_bootstraps):
        ...
        if len(unique_y_sample) < len(np.unique(y_train)):
            ...
            # change here to extract probabilities of the positive class for multi-class calculation
            auc = roc_auc_score(y_sample, y_proba[:, 1], multi_class='ovr')
...
new_data = pd.read_csv('2024.12.06-Solucoes compostas.csv', delimiter=';', encoding='latin-1')
...
X_new = new_data.iloc[:, 3:9].replace(',', '.', regex=True).astype(float)  # Extract X variables

# Apply Savitzky-Golay filter with a smaller window length, and handle potential NaN values
window_length = min(X_new.shape[0], 5) # ensure the window size is valid
if window_length % 2 == 0: #Ensure the window length is odd.
    window_length = window_length - 1

X_new_smooth = savgol_filter(X_new, window_length=window_length, polyorder=2, axis=0, mode='nearest')

# Standard Normal Variate (SNV) with NaN handling
X_new_snv = (X_new_smooth - np.nanmean(X_new_smooth, axis=0)) / np.nanstd(X_new_smooth, axis=0)

# Impute NaN values with the mean of each column using the imputer fitted on X (training data)
from sklearn.impute import SimpleImputer
imputer = SimpleImputer(strategy='mean') # Impute with mean

# Fit the imputer on the training data before transforming the test data
imputer.fit(X) # Fit on training data X

X_new_snv_imputed = imputer.transform(X_new_snv) # Transform new data using the fitted imputer

X_new_scaled = scaler.transform(X_new_snv_imputed)
